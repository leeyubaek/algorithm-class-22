...
def knapsack01_bf(wgt, val, W):
    ...
    for i in range(2 ** n): # 0 ~ 2n -1 모든 부분집합 조합 탐색
        count += 1
        ...
        for j in range(n):
            if s[j] == 1: # j번째 비트가 1이면 즉 j번째 물건을 선택한 경우
                sumWgt += wgt[j] 
                sumVal += val[j]
                chosen_items.append(j) # 선택된 j 번째 무ㅜㄹ건 인데스 포함
        print("선택된 물검 인덱스:",chosen_items,"/ 총 무게 = ", sumWgt, "/ 총 가치", sumVal)
        ...
        if sumWgt <= W :
            ...
            if sumVal > bestVal:
                ...
                print("-> 가치 최대값 갱신! guswo chleo = ", bestVal)
            else: 
                print(" -> 가치 최대값 갱신 없음")
        else:
            print("-> 배낭 용량 초과! 제외")
...
print("최대 가치: ",bestVal,"선택된 물건 :",bestSet)
print("="*100)

#=================================
# 3. 알고리즘 설계 전략: 탐욕적 기법(greedy algoritm)
#=================================
...
def coin_change_greedy(coins, amount) :  # O(nlog n)
    ...
    if remain == 0:
        return total_count,result #사용돤 동전 개수와 조합 반환 
    else:
...
def knapsackFrac_greedy(weights,values,W):
    ...
    # 단계 3 : Greedy 채우기 O(n)
    bestVal = 0
    bag_with_items = [] # 가방에 채워지는 물건 가격

    for ratio, wgt, val,idx in items: # 비율이 높은 순서부터 물건을 채워넣기
        if W <= 0: # 배낭이 채워진 경우
            break
        if W >= wgt : # 배낭이 통채로 넣을 수 있는 경우
            W -= wgt # 넣은 후 남은 용량
...
print(f"테스트 데이터: 최재 가치 = {bestVal}, / 가방에 넣은 물건 기록 {item}")