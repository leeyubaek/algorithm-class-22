...
def tree_height(root): # 이진트리의 높이 계산
	# 재귀적 정의: 트리가 비어있으면 0, 아니면 왼쪽과 오른쪽 서브트리의 높이 중 큰 값 + 1
	if root is None: # 재귀호출 종료 조건
		return 0
...
def count_edges(root): # 이진 트리의 총 간선(엣지, 연결죈 링크) 수 구하기
	# 간선의 수 = 노드의 수 - 1
	nodes = count_nodes(root) # 노드의 수
	return max(0, nodes-1) # 노드가 0인 경우가 음수가 되지 않도록 0 반환
...
def min_nodes_in_binary_tree(k): # 높이가 k인 포화 이진 트리의 최소 노드 개수 구하기 (루트레벨 = 1)
...
def max_nodes_in_binary_tree(k): # 높이가 k인 포화 이진 트리의 최대 노드 개수 구하기 (루트레벨 = 1)
	return full_binary_tree_nodes(k)
	
def count_none_links(root): # 이진트리에서 노드의 개수 N 이라면 연결되지 않은 링트 수 구하기
