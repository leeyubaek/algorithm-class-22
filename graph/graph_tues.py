...
def MSTPrim(vertex, adj, INF = 999):
    n = len(vertex)
    dist = [INF] * n        # 각 정범까지의 최소 거리
    selected = [False] * n  # MST의 정점 포함 여부 기록
    dist[0] = 0             # 시작 정범의 거리 = 0
    
    # helper 함수 정의
    def getMinVertx(dist, selected):
    ...
    for _ in range(n):
        ...
        # 거리 겡신: 그 정점과 연결된 간선들으로 다른 정점들의 거리(dist) 갱신
        for v in range(n):
