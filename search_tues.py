...
def linear_search(A, key, low=0, high=None):
    if high is None:
...
def sequential_search_transpose(A, key, low=0, high=None):
    if high is None:
        high = len(arr) - 1 # high 기본값 처리
    count = 0 # 비교 연산 횟수 카운트
    for i in range(low, high + 1): # low ~ high까지 순차 탐색
...
def binary_search_iter(A, key, low=0, high=None):
    if high is None:
        high = len(arr) - 1     # high 기본값 처리
    count = 0                   # 비교 연산 횟수 카운트
    left, right = low, high     # 탐색 구간 초기화
    mid = (left + right) // 2   # 중앙 인덱스 값
...
def interpolation_search(A, key, low=0, high=None):
    if high is None:
        high = len(arr) - 1     # high 기본값 처리
    count = 0                   # 비교 연산 횟수 카운트
    left, right = low, high     # 탐색 구간 초기화
