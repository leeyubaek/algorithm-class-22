...
def selection_sort(arr): # 선택 정렬
    ...
    for i in range(n-1): # i번째 위치에서 최소값 삽입
        least_idex = i # 최소값 인덱스
        for j in range(i+1,n): # 미정렬된 구간 탐색
            if a[j] < a[least_idex]: # 더 작은 값을 발견
                least_idex = j #작은값 인덱스 갱신
        
        a[i], a[least_idex] = a[least_idex], a[i] # i번째 위치와 최소값 위치 교환
    return a 
...
def quick_sort(arr, left, right): # 퀵 정렬
    ...
    if left < right: # # 정렬한 원소가 2개 이상인 경우
...
def radix_sort(arr, bucket_size = 10): # 기수 정렬
    ...
    while max_val // factor > 0 : # 최대값의 모든 자리수에 대해 반복
        ...
        print(f"기수정렬 중간 단계 (factor = {factor}): {arr[:100]} ...") # 중간 단계 출력(처음~)
